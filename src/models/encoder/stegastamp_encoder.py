...
class StegaStampEncoderV2(nn.Module):
    def __init__(self, image_shape: Tuple[int, int, int], secret_len: int = 100):
        ...
        self.conv2 = Conv2D(32+secret_len, 32, 3, activation='relu', strides=2)
        self.conv3 = Conv2D(32+secret_len, 64, 3, activation='relu', strides=2)
        self.conv4 = Conv2D(64+secret_len, 128, 3, activation='relu', strides=2)
        self.conv5 = Conv2D(128+secret_len, 256, 3, activation='relu', strides=2)
        self.up6 = Conv2D(256, 128, 2, activation='relu')
        self.conv6 = Conv2D(256, 128, 3, activation='relu')
        self.up7 = Conv2D(128, 64, 2, activation='relu')
        self.conv7 = Conv2D(128, 64, 3, activation='relu')
    ...
    def forward(self, image, mask, secret, normalize: bool = False):
        ...
        conv2 = self.conv2(torch.cat([conv1, secret.reshape(*secret.shape, 1, 1).repeat(1, 1, *conv1.shape[-2:])], dim=1))   # 128
        conv3 = self.conv3(torch.cat([conv2, secret.reshape(*secret.shape, 1, 1).repeat(1, 1, *conv2.shape[-2:])], dim=1))   # 64
        conv4 = self.conv4(torch.cat([conv3, secret.reshape(*secret.shape, 1, 1).repeat(1, 1, *conv3.shape[-2:])], dim=1))   # 32
        conv5 = self.conv5(torch.cat([conv4, secret.reshape(*secret.shape, 1, 1).repeat(1, 1, *conv4.shape[-2:])], dim=1))   # 16
        up6 = self.up6(F.pad(F.interpolate(conv5, scale_factor=(2, 2), mode='nearest'), [0, 1, 0, 1]))
        merge6 = torch.cat([conv4, up6], dim=1)
        conv6 = self.conv6(merge6)
        up7 = self.up7(F.pad(F.interpolate(conv6, scale_factor=(2, 2), mode='nearest'), [0, 1, 0, 1]))
