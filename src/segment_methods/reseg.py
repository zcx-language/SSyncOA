...
def seg_image(classname,path,total,batch_size,output_path):
    ...
    for i in range(total//batch_size):
        try:
            imgs=torch.stack([transforms.ToTensor()(Image.open(os.path.join(path,fn)).convert('RGB').resize((224, 224))) for fn in files[i*batch_size:(i+1)*batch_size]],0)
            ...
            for j in range(mask.shape[0]):
                size = Image.open(os.path.join(path,files[i*batch_size+j])).convert('RGB').size
                save_mask = cv2.resize(mask[j],size,interpolation=cv2.INTER_NEAREST)
                cv2.imwrite(os.path.join(output_path,files[i*batch_size+j]),save_mask)
...
if __name__=="__main__":
    ...
    if num_gpus>1:
        ...
        seg_image('anything',args.input_dir,args.samples,args.batch_size,output_path)
